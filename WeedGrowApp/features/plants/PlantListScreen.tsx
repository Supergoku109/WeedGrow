  ...
  return (
    ...
    >
      <ThemedView style={styles.container}>
        ...
        {filtersVisible && (
          <View style={styles.filtersContainer}>
            ...
            <View style={styles.chipRow}>
              <Chip selected={!statusFilter} onPress={() => setStatusFilter(null)} accessibilityLabel="Clear status filter">
                All
              </Chip>
              {['active', 'archived', 'harvested', 'dead'].map((opt) => (
                <Chip key={opt} selected={statusFilter === opt} onPress={() => setStatusFilter(opt)}>
                  {opt}
                </Chip>
              ))}
            </View>
            ...
            <View style={styles.chipRow}>
              ...
              {['outdoor', 'greenhouse', 'indoor'].map((opt) => (
                <Chip key={opt} selected={envFilter === opt} onPress={() => setEnvFilter(opt)}>
                  {opt}
                </Chip>
              ))}
            </View>
            ...
            <View style={styles.chipRow}>
              ...
              {['pot', 'ground'].map((opt) => (
                <Chip key={opt} selected={plantedFilter === opt} onPress={() => setPlantedFilter(opt)}>
                  {opt}
                </Chip>
              ))}
            </View>
            ...
            <View style={styles.chipRow}>
              <Chip selected={!trainingFilter} onPress={() => setTrainingFilter(null)}>
                All
              </Chip>
              {['LST', 'Topping', 'SCROG'].map((opt) => (
                <Chip key={opt} selected={trainingFilter === opt} onPress={() => setTrainingFilter(opt)}>
                  {opt}
                </Chip>
              ))}
            </View>
        ...
        {error && (
          <ThemedText style={[styles.errorText, { color: Colors[theme].tint }]}>❌ Error: {error}</ThemedText>
        )}
        {!loading && !error && plants.length === 0 && <ThemedText>No plants found.</ThemedText>}
        {!loading && !error && filteredPlants.length > 0 && (
          <FlatList
            data={filteredPlants}
            keyExtractor={(item) => item.id}
            renderItem={({ item }) => <PlantCard plant={item} weather={weatherMap[item.id]} />}
          />
        )}
